...
struct options {
  ...
  static void usage(std::string const &prog_name,
...
void memcheck() {
  std::cout << "In " << __func__ << " at " << (void *)_THIS_IP_
            << ", called from " << (void *)_RET_IP_ << std::endl;
...
int main(int argc, char **argv) {
  ...
  if (opts.testcase == "memcheck")
    memcheck();
  else if (opts.testcase == "lock")
    lock();
  else
    all();
  std::cout << "In " << __func__ << " at " << (void *)_THIS_IP_
...
//////////////////////////////////////////////////////////////////////////////
//									     //
//	[alison@hildesheim src (master)]$ g++ -O2 -g -ggdb -Wall -Wextra
//-fsanitize=undefined probe.cc -o probe//
